...
if (__ou_ui && typeof __ou_ui.bindPregameStart === "function"){
  ...
  const BGM = (() => {
    // Tracks are local files (uploaded). Single playlist (no battle/peace switching).
    ...
    function findTrackIndexBySrc(src){
      if (!src) return -1;
      for (let i=0;i<allTracks.length;i++){
        if (src.endsWith(allTracks[i])) return i;
      }
      return -1;
    }

    function rebuildOrder(keepCurrent=true){
      const base = allTracks.slice();
    ...
    return { audio, start, next, prev, toggle, setVol, monitor, mountUI, toggleShuffle, toggleRepeat };
  })();
