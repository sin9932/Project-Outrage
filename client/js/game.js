...
// Example ref: community tables show ~13s at 1 factory for 1000-cost, ~6s at 2, ~4s at 3. (rounded)
  ...
  // expose tuning map so modular sprite modules can reuse the same offsets
  window.PO = window.PO || {};
  window.PO.SPRITE_TUNE = SPRITE_TUNE;
...
function onBuildingPlaced(b){
  ...
  // Modular building sprites/anim hook
  try{ window.PO && window.PO.buildings && window.PO.buildings.onPlaced && window.PO.buildings.onPlaced(b); }catch(_e){}
  }
...
function spawnTurretMGTracers(shooter, target){
  ...
  function destroyBuilding(b, cause={}){
    ...
    // Modular building anim hook (destroy)
    try{ window.PO && window.PO.buildings && window.PO.buildings.onDestroyed && window.PO.buildings.onDestroyed(b, cause); }catch(_e){}
...
function sellBuilding(b){
    ...
    // Modular building anim hook (sell)
    try{ window.PO && window.PO.buildings && window.PO.buildings.onSold && window.PO.buildings.onSold(b); }catch(_e){}
...
const keys=new Set();
  ...
  window.addEventListener("keydown",(e)=>{
    ...
    if (TUNER.on){
      ...
      if (k === "t"){
        // Cycle target kind (ex: HQ -> Barracks) while tuner is active
        try{
          const list = (window.PO && window.PO.buildings && window.PO.buildings.tunerKinds) ? window.PO.buildings.tunerKinds() : ["hq","barracks"];
          const i = Math.max(0, list.indexOf(TUNER.targetKind));
          TUNER.targetKind = list[(i+1) % list.length];
          _updateTuneOverlay();
          toast("target: " + TUNER.targetKind);
        }catch(_e){
          TUNER.targetKind = (TUNER.targetKind==="hq") ? "barracks" : "hq";
          _updateTuneOverlay();
        }
        e.preventDefault();
        return;
  ...
  canvas.addEventListener("mousemove",(e)=>{
    ...
    if (TUNER.on && TUNER.dragging){
      ...
      if (bSel){
        ...
        let img = cfg && cfg.img;
        let crop = (cfg && cfg.crop) ? cfg.crop : { x:0, y:0, w:(img?img.naturalWidth:1), h:(img?img.naturalHeight:1) };
        // Atlas-based buildings (handled by buildings.js) may not have BUILD_SPRITE entries.
        try{
          if ((!cfg || !img) && window.PO && window.PO.buildings && window.PO.buildings.getTunerCrop){
            const c2 = window.PO.buildings.getTunerCrop(TUNER.targetKind);
            if (c2 && c2.w && c2.h){ crop = { x:0, y:0, w:c2.w, h:c2.h }; }
          }
        }catch(_e){}
...
function draw(){
    ...
    // Modular building ghost animations (drawn under particles)
    try{ window.PO && window.PO.buildings && window.PO.buildings.drawGhosts && window.PO.buildings.drawGhosts(ctx, cam, {worldToScreen, ISO_X, ISO_Y, TILE}, state); }catch(_e){}
...
let rX = ent.x, rY = ent.y;
      ...
      if (isB){
        ...
        let _drewCustomB=false;
        try{ _drewCustomB = (window.PO && window.PO.buildings && window.PO.buildings.drawBuilding) ? !!window.PO.buildings.drawBuilding(ent, ctx, cam, {worldToScreen, ISO_X, ISO_Y, TILE, drawFootprintDiamond}, state) : false; }catch(_e){ _drewCustomB=false; }
        if (!_drewCustomB){
...
function sanityCheck(){
  ...
  function tick(now){
    ...
    if (running && !gameOver && !pauseMenuOpen){
      ...
      updateSmoke(dt);
      updateBlood(dt);
      // Modular building sprite/anim tick
      try{ window.PO && window.PO.buildings && window.PO.buildings.tick && window.PO.buildings.tick(dt, state); }catch(_e){}
