...
(function(g){
 const H={modelUrl:'../asset/model/harvester/harvester.glb',scale:20,renderSpan:14,wheelRadius:.427,
  turnRate:2.6,unloadSeconds:1,harvestRate:125,dockSpeed:140,departureDelay:.5,intake:{forward:2.80,height:.63},discharge:{forward:-2.43,height:1.40}};
 ...
 H.approach=(b,T)=>{const p=H.port(b);return{x:(p.tx+1.5)*T,y:(p.ty+.5)*T};};
 ...
 H.tickDock=(u,b,dt,c)=>{
  ...
  if(!d){if(Math.hypot(u.x-approach.x,u.y-approach.y)>10)return false;
    ...
    b.dockUnitId=u.id;d=u.harvesterDock={refId:b.id,phase:'enter',started:time};u.path=null;u.flowGoal=null;
  }
  u.vx=u.vy=0;
  if(d.phase==='enter'){
    const dx=dock.x-u.x,dy=dock.y-u.y,dist=Math.hypot(dx,dy);
    if(dist>.5){
      if(!H.drive(u,dx,dy,dt,dir8))return false;
      const step=Math.min(dist,H.dockSpeed*dt),nx=u.x+dx/dist*step,ny=u.y+dy/dist*step;
      if(!canMove(nx,ny))return false;
      u.vx=(nx-u.x)/Math.max(dt,.001);u.vy=(ny-u.y)/Math.max(dt,.001);u.x=nx;u.y=ny;
      if(dist>step+.5)return false;
    }
    d.phase='turn';d.started=time;return false;
  }
  if(d.phase==='turn'){
    if(!H.drive(u,1,0,dt,dir8))return false;
    d.phase='unload';d.started=time;d.initial=u.carry;d.paid=0;
    b._activeT0=time;b._activePulse=(b._activePulse||0)+1;return false;
  }
  if(d.phase==='unload'){
  ...
  if(d.phase==='close'&&time-d.started>=H.departureDelay){b.dockUnitId=null;u.harvesterDock=null;return true;}
  return false;
 };
