...
function pose(u, time) {
  ...
  if(u.kind==='factory'){
    ...
    model.getObjectByName('RoofL').rotation[F.roofAxis]=roof;model.getObjectByName('RoofR').rotation[F.roofAxis]=-roof;
...
export const ready = (async () => {
  ...
  try {
    ...
    const fc=await fetch(new URL('../asset/model/factory/contract.json?v=3',import.meta.url)).then(r=>{if(!r.ok)throw Error('Factory contract unavailable');return r.json();});
    if(fc.revision!==3||fc.exitLocal[0]!==window.OUFactory.portalX||fc.roofAxis!==window.OUFactory.roofAxis||JSON.stringify(fc.footprint)!==JSON.stringify(window.OUFactory.footprint)||fc.modelYaw!==window.OUFactory.modelYaw||fc.exitHeading!==window.OUFactory.heading||fc.worldUnitsPerMetre!==window.OUFactory.scale)throw Error('Factory exit axis contract mismatch');
    ...
    const contract=await fetch(new URL('../asset/model/mcv/contract.json?v=8',import.meta.url)).then(r=>{if(!r.ok)throw Error('MCV contract unavailable');return r.json();});
    if(contract.revision!==8||contract.runtimeSeconds!==window.OUMCV.seconds||contract.worldUnitsPerMetre!==window.OUMCV.scale||contract.packedRearMetres!==window.OUMCV.packedRearMetres||contract.modelScale!==window.OUMCV.modelScale||contract.up!=='+Y'||contract.heading!=='+Z')throw Error('MCV model dimensions/timing contract mismatch');
