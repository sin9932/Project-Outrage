...
(async()=>{const b=await bt.launch({headless:true,...(engine==='chromium'?{executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe'}:{})});try{
 ...
 await p.evaluate(()=>{u.alive=false;g.state.selection.clear();window.yard=g.addBuilding(0,'hq',18,18);g.mcv.tick=()=>{};g.cam.zoom=.85;g.state.colors.player='#c82cad';g.centerCameraOn(yard.x,yard.y);});
