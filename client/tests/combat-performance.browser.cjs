...
const chromium=require(process.env.OUTRAGE_PLAYWRIGHT)[engine];
const fs=require('fs'),assert=require('node:assert/strict');
const count=Number(process.env.OUTRAGE_STRESS_COUNT||48);
let browser;
(async()=>{
 browser=await chromium.launch({headless:true,...(engine==='chromium'&&(process.env.OUTRAGE_CHROME||process.platform==='win32')?{executablePath:process.env.OUTRAGE_CHROME||'C:/Program Files/Google/Chrome/Application/chrome.exe'}:{})});
 ...
 if(process.env.OUTRAGE_FOG!=='1')await page.locator('#fogOff').check();
 ...
 await page.screenshot({path:process.argv[2]+'.png'});fs.writeFileSync(process.argv[2],JSON.stringify({startup,setup,result,errors},null,2));
