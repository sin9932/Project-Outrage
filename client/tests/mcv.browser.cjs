...
(async()=>{const b=await bt.launch({headless:true,...(engine==='chromium'?{executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe'}:{})});try{
 ...
 await p.evaluate(()=>{repacked.alive=false;window.fac=g.addBuilding(0,'factory',9,10);fac._placedAt=-100;g.state.primary.player.factory=fac.id;window.dep=g.addBuilding(0,'repair',9,16);dep._placedAt=-100;g.state.debug.fastProd=true;});
 ...
 await p.evaluate(()=>{dep=g.addBuilding(0,'repair',9,16);dep._placedAt=-100});
 await p.waitForFunction(()=>g.units.some(u=>u.alive&&u.kind==='mcv'&&u._factoryBornFrom===fac.id),null,{timeout:20000}).catch(async e=>{console.log('DIAGNOSTIC',JSON.stringify(await p.evaluate(()=>({q:fac.buildQ,dispatch:fac._factoryDispatch,units:g.units.map(u=>({kind:u.kind,x:u.x,y:u.y,from:u._factoryBornFrom,alive:u.alive})),state:g.state.gameOverPending,time:g.state.t,fac:{x:fac.x,y:fac.y,alive:fac.alive},dep:{alive:dep.alive},can:OUTech.canUnit(g.buildings,0,'mcv',g.state.t)}))));console.log(errors);throw e;});assert(await p.evaluate(()=>{const u=g.units.find(u=>u.alive&&u.kind==='mcv'&&u._factoryBornFrom===fac.id);return u.x-OUMCV.packedRearMetres*OUMCV.modelScale*OUMCV.scale>fac.x+OUFactory.portalX*OUFactory.scale;}),'MCV rear must clear the factory before independent rendering');console.log('PRODUCTION_OK');
